...
function recipeSearch(recipes, query) {
  let filteredRecipes = recipes.filter(recipeSearchCallBack);
  return filteredRecipes.sort((a, b) => a.name.localeCompare(b.name));
  function recipeSearchCallBack(recipe) {
    if (recipe.name.toLowerCase().includes(query)) return true;

    if (recipe.description.toLowerCase().includes(query)) return true;

    if (recipe.tags.some((tag) => tag.toLowerCase() === query)) return true;
...
function searchHandler(e) {
  e.preventDefault();
  let query = this.searchKeyword.value.toLowerCase();
  let recipeList = recipeSearch(recipes, query);
...
function recipeTemplate(recipe) {
  let tagsHTML = recipe.tags.map(tagTemplate).join("");
  ...
  return `<article>
        ...
        <section class="food-data">
          ...
          <div class="food-title">
            <p class="food-name">${recipe.name}</p>
            ${ratingHTML}
          </div>
...
function renderRecipes(recipeList) {
  let recipe = getRandomListItem(recipeList);

  let recipeHTML = recipeList.map(recipeTemplate).join("");
